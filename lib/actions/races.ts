 ...
 *  YOUTUBE_API_KEY to be set. Each call costs real API quota (100 of the
...
export async function findRaceVideoOnYoutube(raceId: string, query: string): Promise<YoutubeSearchResult> {
  const supabase = await requireAdmin();
  ...
  if (!res.ok) {
    return { ok: false, error: data?.error?.message || "YouTube search failed." };
  }
  const videoId: string | undefined = data?.items?.[0]?.id?.videoId;
  if (!videoId) {
    return { ok: false, error: `No matching video found for "${query}". Try pasting a URL manually instead.` };
  }
