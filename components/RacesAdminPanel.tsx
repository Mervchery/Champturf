...
function ReplayVideoEditor({ race, notify, onChanged }: { race: Race; notify: (m: string) => void; onChanged: () => void }) {
  ...
  const [query, setQuery] = useState(`${race.name} ${race.race_date}`);
  ...
  async function autoFind() {
    ...
    setSearching(true);
    try {
      const result = await findRaceVideoOnYoutube(race.id, query.trim());
      if (result.ok) {
  ...
  return (
    <div className="panel mt-4">
      ...
      <p className="text-xs opacity-50 mt-2.5">
        Auto-find calls the YouTube Data API (needs YOUTUBE_API_KEY set — see README) and saves the top result for whatever query is above — edit it if the default doesn't match. Each click uses real API quota, so it's manual, not automatic.
      </p>
    </div>
  );
}
